...
public class CompanionService extends Service {
    ...
    @Override public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        NowState.start(this);
        startForeground(NOTIFICATION_ID, buildNotification("已启动，等待砚团命令"));
        if (intent != null) {
    ...
    public static void handleCommandBody(Context ctx, String body, String serverUrl, String token) {
        try {
            ...
            if (raw instanceof String) {
                String s = (String) raw;
                if ("peek".equals(s)) executeCommand(ctx, "", "peek", "", "", 0,0,0,0,0,0,350,0,0,"砚团", "砚团截图", true, serverUrl, token);
                return;
            }
            ...
            String title = cmd.optString("title", "砚团提醒");
            String message = cmd.optString("message", cmd.optString("text", title.length() > 0 ? title : "砚团闹钟"));
    ...
    private static void executeSequence(Context ctx, String id, JSONObject cmd, String serverUrl, String token) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            try {
                ...
                String title = step.optString("title", "砚团提醒");
    ...
    private static boolean showReminderNotification(Context ctx, String title, String message, int notificationId) {
        try {
            ...
            String safeTitle = (title == null || title.trim().isEmpty()) ? "砚团提醒" : title.trim();
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                NotificationChannel channel = new NotificationChannel(REMINDER_CHANNEL_ID, "砚团悬浮横幅提醒", NotificationManager.IMPORTANCE_HIGH);
    ...
    private static boolean setAlarm(Context ctx, int hour, int minute, String message, boolean vibrate, boolean skipUi) {
        ...
        try {
            ...
            i.putExtra(AlarmClock.EXTRA_MESSAGE, message == null || message.length() == 0 ? "砚团闹钟" : message);
    ...
    private void createNotificationChannel() { if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) { NotificationManager nm = getSystemService(NotificationManager.class); NotificationChannel channel = new NotificationChannel(CHANNEL_ID, "砚团", NotificationManager.IMPORTANCE_LOW); channel.setDescription("砚团正在等待你授权的截图与手机动作请求"); nm.createNotificationChannel(channel); NotificationChannel reminder = new NotificationChannel(REMINDER_CHANNEL_ID, "砚团悬浮横幅提醒", NotificationManager.IMPORTANCE_HIGH); reminder.setDescription("来自砚团的悬浮横幅、生活提醒与回家模式"); reminder.setLockscreenVisibility(Notification.VISIBILITY_PUBLIC); reminder.enableVibration(true); nm.createNotificationChannel(reminder); } }
    private Notification buildNotification(String text) {
        ...
        return builder.setContentTitle("砚团运行中")
                .setContentText(text)
